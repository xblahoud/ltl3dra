...
void do_intersect_sets(int *l, int *l1, int *l2, int type) /* makes the intersection of two sets and save it in l */
{
  int i;
  for(i = 0; i < set_size(type); i++)
    l[i] = l1[i] & l2[i];
...
void print_set_out_and_mark(std::ostream& out, int *l, int type, int *m) /* prints the content of a set to stream out*/
{
  int i, j, start = 1;
  if(type != 1) out << "{";
  for(i = 0; i < set_size(type); i++)
