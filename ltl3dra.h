...
typedef struct GState {
  ...
  int incoming;
  cset *nodes_set;
//  std::map<cset, std::set<cGTrans> >* trans;
  cGTrans* trans;
//  struct GTrans *trans;
  struct GState *nxt;
  struct GState *prv;
...
void do_intersect_sets(int *, int *, int *, int);
...
class cset
{
 public:
   ...
   // Intersects two sets from input and result save to this->s
   void intersect_sets(const cset &l, const cset &r);
   void intersect_sets(const cset &l, int* r);
   // Substract r from l and save to this->s
   void substract(const cset &l, const cset &r);
...
inline int cset::size(void) const {
  return size_set(s, type);
}
...
#ifdef DRA
    bool check_dominance(ATrans *t, cset *t_to, cset* fin, int &state_trans, GState* s);
    bool determinize(ATrans *t, cset *t_to, cset* fin, int &state_trans, GState* s);
#endif
